...
struct ncbNativeClassMethodBase : public tTJSDispatch {
	...
	ncbNativeClassMethodBase(MethodTypeT t) : _type(t), _name(0) {
		_imethod = this;
		switch (t) { //
//		case nitClass:    _name = TJS_W("Class");    break; //
...
private:
	...
	struct paramsFunctor {
		...
		/*
			struct ArgsConvT {
				ncbToValueConvertor<ARGS::Arg1Type> t1; //
				ncbToValueConvertor<ARGS::Arg2Type> t2; //
				  :
				ncbToValueConvertor<ARGS::Arg[ARGC]Type> tARGC; //
			}; */
