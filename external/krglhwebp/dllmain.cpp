...
void TVPLoadHeaderWEBP(void* formatdata, IStream_ *src, iTJSDispatch2** dic) {
...
static int WebPIStreamWrite(const uint8_t* data, size_t data_size, const WebPPicture* pic)
{
	...
	((IStream_*)(pic->custom_ptr))->Write(data, data_size, &bytes_written);
...
void TVPSaveAsWebP(void* formatdata, void* callbackdata, IStream_* dst, const ttstr & mode, tjs_uint width, tjs_uint height, tTVPGraphicSaveScanLineCallback scanlinecallback, iTJSDispatch2* meta )
{
	if (height == 0 || width == 0)
	{
